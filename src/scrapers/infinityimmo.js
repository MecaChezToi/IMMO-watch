...
const LIST_URL = "https://www.infinity-immo.be/index.php?p=listerBiens&action=V";

async function scrapeInfinityImmo(criteria) {
  const results = [];
  ...
  try {
    ...
    $("a").each((_, el) => {
      ...
      if (!idMatch) return;
      const id = `infinityimmo-${idMatch[1]}`;
      ...
      results.push({
        ...
        url: href.startsWith("http") ? href : `https://www.infinity-immo.be${href}`,
    ...
    console.error("[infinityimmo] erreur:", err.response?.status || err.message);
  }

  return results;
}

module.exports = { scrapeInfinityImmo };
