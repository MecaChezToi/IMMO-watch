...
const LIST_URL = "https://www.ecoimmo.be/chercher-un-bien.php?purpose=1&typebien=1";

async function scrapeEcoImmo(criteria) {
  const results = [];
  ...
  try {
    ...
    $("a").each((_, el) => {
      ...
      results.push({
        ...
        url: href.startsWith("http") ? href : `https://www.ecoimmo.be${href}`,
    ...
    console.error("[ecoimmo] erreur:", err.response?.status || err.message);
  }

  return results;
}

module.exports = { scrapeEcoImmo };
